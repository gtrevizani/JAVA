package cad03_topicos_avancados.pratica._06_04_desafio_collections;
/*
O custo com políticos no Brasil é um dos mais altos no mundo.
O valor do contra-cheque de um político corresponde a cerca de um quarto de tudo o que recebem por meio de inúmeras
verbas e auxílios extraordinários.

Você, político honesto e programador Java, resolveu desenvolver um software para calcular os salários dos políticos por
partido e cargo. Esse software irá lhe ajudar a propor mudanças dentro do governo.

As classes Cargo e Politico foram fornecidas para você pelo Ministério Público Eleitoral.
A classe Cargo representa o cargo de um político, como por exemplo "vereador", "deputado estadual" ou "prefeito". Um
cargo possui uma descrição e o valor do salário.
...
Agora que você já tem as classes Cargo e Politico, precisa criar uma outra classe para organizar os políticos e seus
partidos e calcular os gastos totais dos salários.
Para sua sorte, estamos entregando para você a classe Governo, que está quase pronta. As únicas coisas que faltam são as
implementações os métodos calcularGastosComSalario() e calcularGastosComSalarioParaCargo().
...
A classe Governo possui uma variável de instância para armazenar os políticos por partido. Para isso, é usado um mapa
(interface Map).
private Map<String, List<Politico>> partidosPoliticos;
Essa linha está declarando um mapa (do Collections Framework) onde a chave é do tipo String e o valor do tipo List. Na
chave do mapa, deve-se atribuir a sigla do partido. No valor, atribui-se um objeto do tipo List, como por exemplo um ArrayList.
A instanciação do mapa foi feita para você no construtor de Governo.
public Governo() {
	this.partidosPoliticos = new HashMap<String, List<Politico>>();
}
O método adicionarPolitico() inclui um novo político à lista de políticos, que está dentro do mapa. Se a lista ainda não
 estiver no mapa, o método deve criar um novo registro para a chave igual a sigla do partido. Essas regras já foram implementadas no método para você.
public void adicionarPolitico(String partidoPolitico, Politico politico) {
	...
}
